...
# Referencia: ejemplo llamado ejemplo3.py visto en clasen para poder realizar este programa #*****
...
def source(env, number, interval, RAM ,CPU, lagTime):
    for i in range(number):
        ...
        memoryBlock = random.randint(1,10) # Aleatoriedad de la memoria 
...
def proceso(env, idtag, memoryBlock, RAM, CPU, lagTime, Inst):
    global timeSim, timeDev# Definicion para simular el profiler  para contar el tiempo de ejecucion del simulador
    ...
    with RAM.get(memoryBlock) as req:     
        ...
        while Inst > 0:
            with CPU.request() as reqCPU: # Pide al CPU para poder ejecutar dicha instruccion
                yield reqCPU
